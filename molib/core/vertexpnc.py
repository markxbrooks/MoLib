...
def generate_vertex_pnc(
    positions_array: Vector3Like, normals_array: Vector3Like, c: RibbonColor
) -> VertexPNC:
    """generate vertex pnc"""
    r, g, b = _extract_rgb(c)
    return VertexPNC(
        position=(
            float(positions_array.x),
            float(positions_array.y),
            float(positions_array.z),
        ),
